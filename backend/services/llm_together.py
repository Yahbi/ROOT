"""
Together AI LLM Service — ROOT's connection to Together AI's open-source model hosting.

Uses the OpenAI-compatible API (same client, custom base_url).
Same interface: complete(), complete_with_tools(), stream().

Together AI hosts open-source models (Llama, Mixtral, etc.) with a generous free tier.
"""
...
from backend.config import (
    ...
    TOGETHER_FAST_MODEL,
    TOGETHER_THINKING_MODEL,
)

logger = logging.getLogger("root.llm.together")
...
MODEL_TIERS = {
    ...
    "thinking": TOGETHER_THINKING_MODEL,
}


class TogetherLLMService:
    """Async wrapper around the Together AI API (OpenAI-compatible)."""
    ...
    def __init__(self, api_key: Optional[str] = None, cost_tracker=None, economic_router=None) -> None:
        ...
        self._client = AsyncOpenAI(api_key=key, base_url=TOGETHER_BASE_URL)
    ...
    def _track_usage(self, response, model: str, model_tier: str, method: str = "complete") -> None:
        """Record token usage from a Together AI response."""
    ...
    async def complete(
        ...
        model = MODEL_TIERS.get(model_tier, TOGETHER_DEFAULT_MODEL)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning("Together transient error (attempt %d/%d), retrying in %.1fs: %s",
                                   attempt + 1, MAX_RETRIES, delay, e)
                    await asyncio.sleep(delay)
                else:
                    logger.error("Together API error after %d retries: %s", MAX_RETRIES, e)
                    raise LLMUnavailableError(str(e)) from e
            except Exception as e:
                if "server_error" in str(e).lower() or "502" in str(e) or "503" in str(e):
                    if attempt < MAX_RETRIES - 1:
                        delay = RETRY_BASE_DELAY * (2 ** attempt)
                        logger.warning("Together server error (attempt %d/%d), retrying: %s",
                                       attempt + 1, MAX_RETRIES, e)
                        await asyncio.sleep(delay)
                        continue
                logger.error("Together API error: %s", e)
                return ""
        return ""
    ...
    async def complete_with_tools(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    delay = RETRY_BASE_DELAY * (2 ** attempt)
                    logger.warning("Together tool_use transient error (attempt %d/%d), retrying: %s",
                                   attempt + 1, MAX_RETRIES, e)
                    await asyncio.sleep(delay)
                else:
                    raise LLMUnavailableError(str(e)) from e
            except Exception as e:
                logger.error("Together API error (tool_use): %s", e)
                return "", []
        return "", []
    ...
    async def stream(
        ...
        try:
            ...
            logger.error("Together stream error: %s", e)
            yield f"[Error: {e}]"
